...
func (s *DeliverSmTestSuite) SetupTest() {
	...
	s.valueBytes = []byte{
		0, 0, 0, 223, 0, 0, 0, 5, 0, 0, 0, 0, 0, 0, 98, 100, 0, 1, 1, 57, 49, 57, 56, 49, 56, 53, 50, 48, 50, 56, 53, 0, 1, 1, 83, 72, 65, 65, 68, 73, 0, 1, 1, 1, 0, 0, 1, 1, 1, 1, 132, 105, 100, 58, 55, 97, 52, 52, 97, 97, 98, 97, 45, 51, 51, 54, 102, 45, 52, 97, 57, 50, 45, 57, 53, 48, 50, 45, 100, 100, 49, 48, 54, 97, 97, 55, 51, 54, 57, 102, 32, 115, 117, 98, 58, 48, 48, 49, 32, 100, 108, 118, 114, 100, 58, 48, 48, 49, 32, 115, 117, 98, 109, 105, 116, 32, 100, 97, 116, 101, 58, 50, 51, 49, 49, 50, 51, 49, 57, 51, 55, 53, 56, 32, 100, 111, 110, 101, 32, 100, 97, 116, 101, 58, 50, 51, 49, 49, 50, 51, 49, 57, 51, 56, 48, 48, 32, 115, 116, 97, 116, 58, 68, 69, 76, 73, 86, 82, 68, 32, 101, 114, 114, 58, 48, 48, 48, 32, 116, 101, 120, 116, 58, 0, 30, 0, 36, 55, 97, 52, 52, 97, 97, 98, 97, 45, 51, 51, 54, 102, 45, 52, 97, 57, 50, 45, 57, 53, 48, 50, 45, 100, 100, 49, 48, 54, 97, 97, 55, 51, 54, 57, 102,
	}
}
...
func (s *DeliverSmTestSuite) TestDeliverSM_IEncode() {
	d := DeliverSm{
		...
		tlvs: map[uint16]TLV{
			RECEIPTED_MESSAGE_ID: NewTLVS(RECEIPTED_MESSAGE_ID, s.msgID),
		},
	}
	data, err := d.IEncode()
	assert.Nil(s.T(), err)
...
func (s *DeliverSmTestSuite) TestDeliverSM_SetSequenceID() {
	...
	assert.Equal(s.T(), d.Header.ID, DELIVER_SM)
	...
	tlv, ok := d.tlvs[RECEIPTED_MESSAGE_ID]
	assert.True(s.T(), ok)
