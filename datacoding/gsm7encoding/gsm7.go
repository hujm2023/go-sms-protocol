...
/*
EscapeSequence GSM 7-bit default alphabet and extension table
...
func (g *gsm7Decoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	...
	// by hujiaming.0927: 避免\r造成多一个字节.
	// 在 encode 时，当最后一个字节中，前 7 位都是 0 时，为了避免和 '@'(0x01)混淆，一般会多一个 CR(0000 1101) 用来填充。
	// 填充后，最后一个字节由 0000 000X 变成0001 101X，(X 为 0 或 1)，这样Decode后会多一个字节'\r'，会引起计费问题，这里去掉这个 '\r'。
	// 第一个判断依据：当原文长度经过 encode 之后，长度满足 8*n-1，就会出现上面这个 case。
	// 例1：1234567ahifbewibaimwnfe，长度为 23，没有 escaped 字符，encode 之后长度也为 23，满足 8*n-1，Decode 之后的内容为 "1234567ahifbewibaimwnfe\r"；
	// 例2：1234567ahifbewi[imwnfe，原来长度是 22，encode 后 [ 占两个字节，所以encode后长度是 23，也符合 8*n-1.
...
func (g *gsm7Encoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	...
	// When there are 1 to 6 spare bits in the last octet of a message, these bits are set to
	// zero (these bits do not count as a character but only as a filler).
	...
	// by hujiaming.0927: 当最后一个字节的前置 7 位都是0时，为了区别 0x00('@')，一般会将这 7 位设置为 CR(0x0d)，即换行
...
func Encode(src string) (dst []byte, err error) {
	if len(src) == 0 {
		return nil, nil
	}

	septets := make([]byte, 0, len(src))
	// work with []rune (a.k.a string) instead of []byte
	for _, r := range src {
		if v, ok := forwardLookup[r]; ok {
			septets = append(septets, v)
		} else if v, ok := forwardEscape[r]; ok {
...
func Pack(septets []byte) (dst []byte) {
	dst = make([]byte, int(math.Ceil(float64(len(septets))*7/8)))
	nDst := 0
	nSeptet := 0
	remain := len(septets) - nSeptet
	for remain > 0 {
...
func Unpack(src []byte) (septets []byte) {
	...
	// 例1：1234567ahifbewibaimwnfe，长度为 23，没有 escaped 字符，encode 之后长度也为 23，满足 8*n-1，Decode 之后的内容为 "1234567ahifbewibaimwnfe\r"；
	// 例2：1234567ahifbewi[imwnfe，原来长度是 22，encode 后 [ 占两个字节，所以encode后长度是 23，也符合 8*n-1.
	if len(septets)%8 == 0 && septets[len(septets)-1] == 0x0d {
		septets = septets[:len(septets)-1]
	}
	return septets
}

func Decode(septets []byte) (dst []byte, err error) {
	nSeptet := 0
	builder := bytes.NewBufferString("")
